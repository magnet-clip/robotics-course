...
max_weight = max(w)
weighted_probs = [x / max_weight for x in w]

p_prob = zip(p, weighted_probs) # particles and their probabilities
p_prob.sort(key=lambda (item, prob): prob)

(sorted_p, sorted_w) = zip(*p_prob)

from bisect import bisect
print bisect(sorted_w, 0.000005)

res = []
for i in range(N):
    rnd = random.uniform(0, 1)
    pos = bisect(sorted_w, rnd)
    print pos
    res.append(sorted_p[pos])
